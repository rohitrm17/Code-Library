...
public:
    ...
    void update(int curr, int l, int r, int i, int j, int val){
        if (islazy[curr]) lazyUpdate(curr, l, r);
    ...
    NodeT query(int curr, int l, int r, int i, int j){
        if (islazy[curr]) lazyUpdate(curr, l, r);
        if (j < l || i > r) return NodeT();
        if (i <= l && r <= j) return tree[curr];
        int mid = (l + r) / 2;
        NodeT ans;
...
void solve(){
    ...
    for (int c = 0; c < N; c++){
        ...
        int id = -1;
        if (zeroPositions.size() == 1) id = (*zeroPositions.begin());
        else{
            int last = *(--zeroPositions.end());
            int first = *zeroPositions.begin();
            if ((N - last + first) > K) id = first;
            else{
                int start = first;
                while (true){
        ...
        else tree.update(1, 1, N, id - K, id, -1);
    }
